 ...
 */( function(window, document, $, undefined) {"use strict";
	var W = jQuery(window), D = jQuery(document), F = jQuery.fancybox = function() {
		F.open.apply(this, arguments);
	}, didResize = false, resizeTimer = null, isTouch = document.createTouch !== undefined, isString = function(str) {
	...
	jQuery.extend(F, {
		...
		open : function(group, opts) {
			...
			if(group && !jQuery.isArray(group)) {
				group = group instanceof $ ? jQuery(group).get() : [group];
			}
			F.isActive = true;
		...
		bindEvents : function() {
			var current = F.current, keys = current.keys;
			if(!current) {
				return;
			}
		...
		_start : function(index) {
			var coming = {}, element = F.group[index] || null, isDom, href, type, rez, hrefParts;
			...
			coming = jQuery.extend(true, {}, F.opts, {
				index : index,
				element : element
			}, (jQuery.isPlainObject(element) ? element : coming));
			...
			if(type === 'inline' || type === 'html') {
				if(!coming.content) {
					if(type === 'inline') {
						coming.content = jQuery(isString(href) ? href.replace(/.*(?=#[^\s]+$)/, '') : href);
					} else {
		...
		_preloadImages : function() {
			var group = F.group, current = F.current, len = group.length, item, href, i, cnt = Math.min(current.preload, len - 1);
		...
		_setContent : function() {
			var current = F.current, content = current.content, type = current.type, minWidth = current.minWidth, minHeight = current.minHeight, maxWidth = current.maxWidth, maxHeight = current.maxHeight, loadingBay;
			switch(type) {
				case'inline':
				case'ajax':
				case'html':
					...
					if(current.autoSize) {
						loadingBay = jQuery('<div class="fancybox-wrap ' + F.current.wrapCSS + ' fancybox-tmp"></div>').appendTo('body').css({
				...
				case'swf':
					...
					break;
				case'iframe':
					content = jQuery(current.tpl.iframe.replace('{rnd}', new Date().getTime())).attr('scrolling', current.scrolling).attr('src', current.href);
		...
		_setDimension : function() {
			var wrap = F.wrap, inner = F.inner, current = F.current, viewport = F.getViewport(), margin = current.margin, padding2 = current.padding * 2, width = current.width, height = current.height, maxWidth = current.maxWidth + padding2, maxHeight = current.maxHeight + padding2, minWidth = current.minWidth + padding2, minHeight = current.minHeight + padding2, ratio, height_;
	...
	F.transitions = {
		getOrigPosition : function() {
			var current = F.current, element = current.element, padding = current.padding, orig = jQuery(current.orig), pos = {}, width = 50, height = 50, viewport;
		...
		zoomIn : function() {
			var wrap = F.wrap, current = F.current, effect = current.openEffect, elastic = effect === 'elastic', dim = current.dim, startPos = jQuery.extend({}, dim, F._getPosition(elastic)), endPos = jQuery.extend({
				opacity : 1
			}, startPos);
		...
		zoomOut : function() {
			var wrap = F.wrap, current = F.current, effect = current.openEffect, elastic = effect === 'elastic', endPos = {
				opacity : 0
			};
			if(elastic) {
		...
		changeIn : function() {
			var wrap = F.wrap, current = F.current, effect = current.nextEffect, elastic = effect === 'elastic', startPos = F._getPosition(elastic), endPos = {
				opacity : 1
			};
			startPos.opacity = 0;
			if(elastic) {
		...
		changeOut : function() {
			var wrap = F.wrap, current = F.current, effect = current.prevEffect, endPos = {
				opacity : 0
			}, cleanUp = function() {
	...
	jQuery.fn.fancybox = function(options) {
		var that = jQuery(this), selector = this.selector || '', index, run = function(e) {
