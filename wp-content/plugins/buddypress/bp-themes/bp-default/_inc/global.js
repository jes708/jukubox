...
jq(document).ready( function() {
	...
	// NHF - to alter if someone clicks on friends tab - FRIENDS TAB - code starts here
	...
	jq('li.filter select').change( function() {
		...
		// NHF TEST
		var inst_select = jq('#search-by-instrument'); 
		var instrument = jq('#search-by-instrument option').filter(":selected").val();
		if( inst_select.length == 0 ) { 
			instrument = 'notApplicable'; 
		} 
		else if( typeof instrument === "undefined" ) { 
			instrument = 'all'; 
		}  
		 //alert(instrument); 
		...
		if ( 'friends' == object )
			object = 'members';
		/*NHF TEST*/ //alert(filter); /*END NHF TEST */  
		// NHF ORIGINAL FUNCTION COMMENTED OUT
		//bp_filter_request( object, filter, scope, 'div.' + object, search_terms, 1, jq.cookie('bp-' + object + '-extras') );
		
		bp_filter_request( object, filter, scope, 'div.' + object, search_terms, 1, jq.cookie('bp-' + object + '-extras'), instrument );
		return false;
	});

	/* NHF - custom function - copy of last function */ 
	/* this way, if we change instruments with our instrument selector, it triggers this search */ 

	jq('#search-by-instrument').change( fire_search ) ; 

	function fire_search() {
		//var this = 'li.filter select'; 
		if ( jq('.item-list-tabs li.selected').length )
			var el = jq('.item-list-tabs li.selected');
		else
		...
		var object = css_id[0];
		var scope = css_id[1];
		var filter = jq('li.filter select').val();
		var search_terms = false;
	...
	jq(".friendship-button a").live('click', function() {
		...
		// NHF edit - require approval for cancelling a friendship request
 
		if( thelink.hasClass('pending_friend') ) { 
			 
			var confPend = confirm('Are you sure you want to cancel this request?'); 
			if (confPend == false) { 
				return false; 
			} 
		}
...
/* instrument value - added by NHF */ 
function bp_filter_request( object, filter, scope, target, search_terms, page, extras, instrument ) {
	if ( 'activity' == object )
		return false;

	// NHF MAKE INSTRUMENT AN OPTIONAL ARGUMENT
	...
	/* NHF - figure out where this is going! */ 
	bp_ajax_request = jq.post( ajaxurl, {
		...
		'scope': scope,
		'page': page,
		'extras': extras /* NHF TEST NEW VARIABLE */, 
	...
	function(response)
	{
		...
		jq('.item-list-tabs li.selected').removeClass('loading');
		/* NHF TEST */ //alert(instrument); /* NHF TEST */  
	});
}
