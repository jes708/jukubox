...
*/	
	...
	function my_appointments( $atts ) {
		...
		// NHF EDIT - added _finch_allow_cancel attribute 
		...
		// Make this a form for BP if confirmation is allowed
		// NHF CUSTOMIZATION
		if ( $this->bp && $allow_confirm ) {
			if( $provider == 1 ) { $newClass = "worker"; } 
			else { $newClass = "user"; } 
			$ret .= '<form class="app_form ' . $newClass . '" method="post">';
		}
		// END NHF CUSTOMIZATION
		$ret .= $title;
		...
		if ( $results ) {
			...
			// NHF EDIT - have the table display the client's name and permalink
			//$finch_clientname = $r->name ;
			//$finch_clientid = $r->user;  
			// echo $finch_clientname . ' ' . $finch_clientid;  // end NHF EDIT
				$ret .= '<tr><td class="service_name" ser_id="' . $r->service . '">';
				$ret .= $this->get_service_name( $r->service ) . '</td><td>';
				if ( !$provider )
					$ret .= $this->get_worker_name( $r->worker ) . '</td><td>';
				else  // NHF - changed value below to new value
					//$ret .= $this->get_client_name( $r->ID ) . ' </td><td>';
					$ret .=   bp_core_get_userlink($r -> user) . ' </td><td>';
				$ret .= date_i18n( $this->datetime_format, strtotime( $r->start ) ) . '</td><td class="app_status">';
				$ret .= $r->status;
				$ret .= '</td>';

				// If allowed so, a worker can confirm an appointment himself
				// NHF edit - added class to following input
				if ( $allow_confirm ) {
					if ( 'pending' == $r->status )
						$is_readonly = '';
					else
					...
					$ret .= '<td><input type="checkbox" class="app_confirm" name="app_confirm['.$r->ID.']" '.$is_readonly.' /></td>';
				}
				// NHF EDIT - if they can reject the appointment
				
				if ( $_finch_allow_cancel == 1 ) { 
					
					$ret .= '<td><input type="checkbox" class="app_deny" name="app_deny['.$r->ID.']"  /></td>';
				}  // END NHF
				if ( $gcal ) {
	...
	function services( $atts ) {
		...
		$s .= '</select>';
		// NHF EDIT - get the service button to include the provider id
		...
		$href = apply_filters( 'app_service_href', add_query_arg( array( "wcalendar"=>$wcalendar, "app_service_id" => "'+selected_service+'&app_provider_id='+current_provider" ), $href ) );
		...
		$script .= "window.location.href='".$href.";";
		//$script .= "window.location.href='".$href."&app_provider_id=' + current_provider + '' ;";
	...
	function service_providers( $atts ) {
		...
		$script .= "var selected_worker=$('.app_select_workers option:selected').val();";
		// NHF - edit the url so that includes the get url of the worker
		//$href = 
		// End NHF edit
		$script .= "window.location.href='".$href.";";
		$script .= "});";
	...
	function confirmation( $atts ) {
		...
		$ret .= '<button type="button" class="btn btn-gray appointments-confirmation-cancel-button">'.__('Cancel', 'appointments').'</button>';
		$ret .= '</div>';
		$ret .= '</fieldset></div>';
		...
		if ( is_user_logged_in() || 'yes' != $this->options["login_required"] ) {
			...
			//$script .= 'alert(app_val);';
			$script .= 'var app_val_array = app_val.split(":");';
			$script .= 'var temp_worker_id = app_val_array[2];';
			$script .= 'var temp_student_id = window["finch_user_id"];';
		
			//$script .= '$(this).css("text-align","center").append(wait_img);';
			$script .= 'var checkPendString = "workerId=" + temp_worker_id + "&studentId=" + temp_student_id + "";' ; 			    $script .= 'finch_temp_cell.css("text-align","center").append(wait_img) ;'; 	
			$script .= ' finch_ajax_request("checkPendingAppointment", checkPendString, testSuccess);  ' ;
			$script .= 'function testSuccess(response) { '; 		
			//$script .= 'console.log(response);  '; 		
			$script .= 'if( response != 0 ) { alert("You already have another lesson request pending.  You cannot request more until the teacher approves or cancels the last request");  $(".wait_img").remove(); finch_temp_cell.removeAttr("id");  }   ';  
			$script .= 'else { var thisy = finch_temp_cell; /*$(".appointments-list table td.free, .app_timetable div.free#finch_temp_cell").not(".app_monthly_schedule_wrapper table td.free"); */   /* alert("helloi person");*/  '; 
			//$script .= '$(thisy).css("text-align","center").append(wait_img);';
			$script .= 'var app_value = $(thisy).find(".appointments_take_appointment").val();';
			...
			$script .= '$.post(_appointments_data.ajax_url, pre_data, function(response) {
				      ...
				      $script .= ' thisy.removeAttr("id");  '; //  remove temporary id
				    $script .= ' }  '; // end else 
				$script .='}  '; // end test success
			$script .= '});';
		}
		...
		$script .= 'var user_id = ' . $current_user->ID . ';';
		$script .= 'var user_name = "' . $current_user->display_name . '";';
		//
//		$script .= 'var post_data = {action: "post_confirmation", value: final_value, app_name: app_name, app_email: app_email, app_phone: app_phone, app_address: app_address, app_city: app_city, app_note: app_note, app_gcal: app_gcal, nonce: "'. wp_create_nonce() .'"};';
		$script .= 'var post_data = {action: "post_confirmation", value: final_value, app_name: app_name, app_email: app_email, app_phone: app_phone, app_address: app_address, app_city: app_city, app_note: app_note, app_gcal: app_gcal, user_id: user_id, user_name: user_name,  nonce: "'. wp_create_nonce() .'"};';
		...
		// NHF original commentd out 
		//$script .= '$.post(_appointments_data.ajax_url, post_data, function(response) {
		// NHF - make so can only request one appointment at a time
			
		$script .= '/* alert("You already requested a lesson"); return false;*/ ' ; 		
		$script .= 'var checkPendString = "workerId=1&studentId=1";' ; 		
		$script .= '/* finch_ajax_request("checkPendingAppointment", checkPendString, testSuccess);*/  ' ;
		// NHF ajax nastiness 6/6/13 
		$script .= 'function testSuccess(response) { '; 		
		$script .= 'console.log(response);  '; 		
		$script .= 'if( response != null ) { alert("You already have another lesson request pending.  You cannot request more until the teacher approves or cancels the last request"); window["finch_pend_confirm"] = "more_pend"; } '; 
		$script .= ' } ' ; // end test success
		$script .= 'if( window["finch_pend_confirm"] == "more_pend" ) { return false; }  ' ; 
		 // end NHF 		
		$script .= '$.post(_appointments_data.ajax_url, post_data, finch_return_confirmation, "json" ); 
			function finch_return_confirmation(response) { 
						$(".wait_img").remove(); 
						/* NHF CODE - deal with response */ 
						/*console.log(response);*/  
						var response_object = response; 
						var finch_cell = response_object.cell; 
						var finch_app_id = response_object.app_id;
						var finch_price = response_object.price; 
						var finch_service = encodeURIComponent(response_object.service_name);
						var finch_data_string = "AppCell=" + finch_cell + "&AppId=" + finch_app_id + "&AppPrice=" + finch_price + "&AppService=" + finch_service + ""; 
						/* END NHF CODE */ 
						if ( response && response.error ) {
							alert(response.error);
						}
							...
							if ( response.gcal_url != "" ) {
								...
								generate_email(finch_data_string, "refresh");
								/* NHF - line below commented out - */ 
								/* causes $.ajax to not function properly */ 
								/* refreshing added to generate_email() function */  
								/*window.location.href=app_location();*/ 
							}
						    ...
						    /* console.log(response); */  
						    function nhf_paypal_lesson(wait_img, finch_data_string, response, callback ) { 	
							/*Added by NHF */ 
							$(".appointments-confirmation-cancel-button").after(wait_img);
							generate_email(finch_data_string, "no refresh");
							$(".wait_img").remove(); 
							/* end NHF added */ 
							...
							/* NHF code - change service name to include lesson ID, teacher and student name */
							var appointment_id = response.app_id; 
							var worker_id = response.worker_id;
							var worker_name = response.worker_name; 
							var user_id = response.user_id; 
							var user_name = response.user_name; 
							var app_finch_time = response.start_date;    
							var full_service_message = response.service_name + " ID num " + appointment_id + " between user " + user_name + "(" + user_id +") and " + worker_name + "(" + worker_id + ") on " + app_finch_time + ""; 

							/* end NHF code */ 
							/* original - var new_val2 = old_val2.replace("SERVICE",response.service_name); */ 
							var new_val2 = old_val2.replace("SERVICE",full_service_message);
							...
							if ( response.mp == 1 ) {
								...
								/*$(".appointments-paypal").show( function() {}, responde);*/ 
								function responde() { 
									/*$(".appointments-paypal form").delay(3000).submit();*/  
								} 
								/*$(".appointments-paypal form").submit();*/  							
							}
							callback(); 
						    }  /* end function */  
						    nhf_paypal_lesson(wait_img, finch_data_string, response, function() { 
							alert("Forwarding for Payment!"); 
							window.setTimeout(submitIt, 2000); 
							function submitIt() { 
								$(".appointments-paypal form").submit();
							} 
							/*$(".appointments-paypal form").delay(7000).submit();*/  
							/*alert("Success?");*/  
						    }); /*end paypal_lesson */ 		
						}
						else{
	...
	function post_confirmation() {
		...
		// NHF added custom formatted date
...
//		$finch_start_date = $start; 
		// end NHF 
		// NHF - added worker name to json object
		$worker_finch_data = get_userdata( $worker );
		$worker_finch_name = $worker_finch_data->display_name;  
		// 
		if ( is_user_logged_in( ) ) {
			$user_id = $current_user->ID;
		...
		// NHF added real name
			$user_finch_name = $current_user->display_name; 
		// end NHF 	
			$user_name = $userdata->display_name;
			if ( !$user_name )
				$user_name = $userdata->first_name . " " . $userdata->last_name;
		...
		if ( isset( $this->options["payment_required"] ) && 'yes' == $this->options["payment_required"] ) {
			die( json_encode( 
						...
						// NHF - ajax additions
							"user_id"			=> $user_id,
							"user_name"			=> $user_finch_name, 
							"start_date"			=> $finch_start_date,
							"worker_name"			=> $worker_finch_name,	
							"worker_id"			=> $worker
	...
	function paypal( $atts ) {
		...
		$form .= '<input class="app_student_name" type="hidden" name="amount" value="" />';
		$form .= '<input class="app_teacher_name" type="hidden" name="amount" value="" />';	
...
//		'title'			=> __('<h3>Our schedule for START</h3>', 'appointments'),
		'title'			=> __('<h3>START</h3>', 'appointments'),
...
*/
	...
	function save_profile( $profileuser_id ) {
		...
		if ( is_object( $this->gcal_api ) ) {
			$kfn = $this->gcal_api->get_key_file( $profileuser_id ). '.p12';
	...
	/**
	 * FINCH CUSTOM METHOD - allow users to confirm apointmetns without resetting schedule
	 */ 
	
	function save_profile_finch_app_conf( $profileuser_id ) {
		global $current_user, $wpdb;
	...
	?>
		...
		<?php if ( isset($this->options["allow_worker_wh"]) && 'yes' == $this->options["allow_worker_wh"] ) { ?>
			...
			<?php } ?>
		<?php } ?>
		<?php if ( isset($this->options["gcal_api_allow_worker"]) && 'yes' == $this->options["gcal_api_allow_worker"] ) { ?>
			<tr>
...
*/
	...
	function bp_init() {
		...
		// NHF EDIT - registered new save profile function
			// the reason for this was that before, when workers confirmed appointments, 
			// it also reset their teaching schedules as well.  I wrote a custom funciton that was
			// a copy of another function that took all these capabilities out.  
		$furl = $_POST['_wp_http_referer'];
		$postmem =  strpos( $furl, 'members'); 
		$postapp = strpos( $furl, 'my-appointments'); 
		if( $postmem != FALSE && $postapp != FALSE ) {
			$this->save_profile_finch_app_conf( $user_id ) ; 
			//wp_die( 'You don\'t have the authority to do this., ' . $furl . ' ' . $postmem . ' ' . $postapp . '', 'appointments' );
		} 
		else { 
		// END NHF TESTS
	...
	// NHF - add new profile page! 
	function tab_template_app_pricing_settings() {
		add_action( 'bp_template_content', array( &$this, 'screen_content_app_pricing_settings' ) );
		bp_core_load_template( apply_filters( 'bp_core_template_plugin', 'members/single/plugins' ) );
	}
	...
	function screen_content_my_app() {
		...
		else   // NHF EDIT - added new shortcode attribute in here
			echo '<br />';
...
//                        } elseif ($raw_half > 0) { 
		...
		if( is_teacher($user_id)===TRUE ) {
			...
			<script>
				...
				//alert(text); 
				jQuery('.my-appointments-confirm').text('Confirm');
				jQuery('.submit input[name=\"app_bp_settings_submit\"]').hide(); 
				...
				jQuery('.app_confirm').click( function() { 
					...
					var conf = confirm('Are you sure you want to confirm this appointment?'); 
					if( conf == false ) { 
						...
						jQuery('.app_form').submit(); 
					} 	
				}); // end app_confirm click 
	
			 	 
				jQuery('.app_deny').click(function() {
					...
					var PostQueryString = 'LessonIdsToCancel=' + cancelString +'&canceller=$user_id&cancelnotice=' + URLnotice + ''; 
					 jQuery.ajax({  
  						type: 'POST',  
						dataType: 'JSON', 
				...
				// highlights appointments that need to be approved
				jQuery('.app_status').each( function() { 

					var app_status = jQuery(this).text();
					var app_service = jQuery(this).parent('tr').children('.service_name').text(); 
					//alert(app_service);  
					//alert(app_status);
					...
					if( ( app_status == 'confirmed')   ) { 
						...
						//jQuery(this).parent('tr').css('border', 'solid red 1px');  
					}  
...
// NHF - new profile tab for naming prices 

function screen_content_app_pricing_settings() {
		// In the future we may use this function without BP too
		if ( function_exists( 'bp_loggedin_user_id') )
			$user_id = bp_loggedin_user_id();
		else {
...
*/
	...
	function _install() {
		...
		if( !$db_version || version_compare( $db_version, '1.1.8', '<' ) ) {
		...
		if ( !$db_version || version_compare( $db_version, '1.2.2', '<' ) ) {
...
********************************
	...
	function send_confirmation( $app_id ) {
		...
		if ( $r != null ) {
			...
			if ( $r->email && $mail_result ) {
				...
				$provider_add_text .= "\n\n\n";
				//NHF CODE - edited the subject line - original was just "New Appointment
				wp_mail( 
						$to,
						$this->_replace( __('[Jukubox] New Appointment You Confirmed','appointments'), $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price), $r->phone, $r->note, $r->address, $r->email ),
	...
	function send_notification( $app_id ) {
		...
		if ( $r != null ) {
			...
			//  attempt to alter emails sent - sprintf( __('The new appointment has an ID %s and you can confirm it using your profile page.  Log in at ' . get_home_url() . '','appointments'), $app_id ),
